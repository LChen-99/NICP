...
namespace nicp {
  ...
  protected:
    ...
    /** \class StatsCalculator2D statscalculatorintegralimage.h "statscalculatorintegralimage.h"
   *  \brief Class for 2D point properties computation.
   *  
   *  This class provides the necessary methods for normals and additional point properties using
   *  KDtree
   */
  class StatsCalculator2D : virtual public StatsCalculator {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  ...
  protected:
    int _minPoints; /**< Minimum number of points to consider a normal valid. */
    float _curvatureThreshold; /**< Curvature threshold for which point normals with higher curvature are not valid. */
    float _worldRadius; /**< Radius in the 2D euclidean space in which neighboring points are selceted for normal computation. */
